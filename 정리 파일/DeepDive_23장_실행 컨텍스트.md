...
실행 컨텍스트 스택에서 bar 함수 실행 컨텍스트가 pop되어 제거되고 foo 실행 컨텍스트가 실행 중인 실행 컨텍스트가 된다. 이때, 렉시컬 환경은 실행 컨텍스트에 의해 참조되기는 하지만 독립적인 객체이므로 bar 함수 실행 컨텍스트가 제거되어도 bar 함수 렉시컬 환경이 즉시 소멸하지 않는다.
