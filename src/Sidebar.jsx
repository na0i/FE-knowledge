...
const sidebarMenus = [
  {
    ...
    desc: [
      {
        id: 0,
        value: "음성을 입력하면 자동으로 텍스트로 변환해줘요:)",
      },
      {
        id: 1,
        value: "영상 음성을 분석해 ai가 자동으로 자막을 생성해줘요:)",
      },
      {
        id: 2,
        value: "음성으로 검색하는 기능이 필요한 경우 사용해 보세요:)",
      },
    ],
  },
  {
      ...
      {sidebarMenus.map((menu) => {
        return (
          <MenuDiv>
            ...
            <MenuDropdowns>
              {menu.items?.map((item) => (
                <MenuDropdown>{item.value}</MenuDropdown>
              ))}
            </MenuDropdowns>
          </MenuDiv>
        );
